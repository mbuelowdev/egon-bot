...
void main() {
  group('ToolRegistry access enforcement', () {
    test('owner can run any tier without preview', () async {
    ...
    test('non-owner dangerous tools escalate to approval', () async {
      ...
      expect(result.isError, isFalse);
      expect(result.json['status'], 'pending_approval');
      expect(tool.executions, 0);

      final pending =
          services.approvals.byId(result.json['approval_id'] as int);
      expect(pending, isNotNull);
      expect(pending!.toolName, 'stub_tool');
    });

    test('previewChange pauses even for the owner', () async {
      final tool = StubTool(preview: 'diff --git a/note.md');
      final services = testServices(tools: [tool]);
      final result = await services.registry.dispatch(
        contextFor(services, owner: true),
        call('stub_tool'),
      ...
      final outcome = await services.approvals.decide(
        id: result.json['approval_id'] as int,
        approved: true,
        actorId: ownerId,
      );
      expect(outcome.name, 'handled');
      expect(tool.executions, 1);
    });
    ...
    test('non-owners see standard + dangerous schemas, not personal', () {
      ...
      expect(userSchemas, containsAll(['stub_tool', 'dangerous_tool']));
      expect(userSchemas, isNot(contains('personal_tool')));
    });
