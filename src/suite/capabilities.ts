...
export const RUNNER_CAPABILITIES_PROMPT = [
  ...
  `- \`disabled\` (Godot's default): nothing is scaled. The running root viewport IS the ${TESTER_VIEWPORT_SIZE} window. The game's \`window/size/viewport_*\` should also be ${TESTER_VIEWPORT_SIZE}, so Godot positions equal click coordinates. If GAME_MAP's WxH differs, that WxH is only the editor design size — write x,y in the ${TESTER_VIEWPORT_SIZE} window, derived from anchors and centering (a centered Control is at ${String(TESTER_VIEWPORT_WIDTH / 2)},${String(TESTER_VIEWPORT_HEIGHT / 2)}), not from editor coordinates.`,
