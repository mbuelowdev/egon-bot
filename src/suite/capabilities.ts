...
export const RUNNER_CAPABILITIES_PROMPT = [
  ...
  "",
  "It can:",
  `- Load any registered scenario by name (\`?egon_scenario={name}\`) and wait for the Godot engine to start before the first step.`,
  ...
  "- Capture a screenshot or a 5–10s video for human proof, as each check's `proof` field says (`screenshot` or `video`). Neither is the pass condition. `video` records the canvas after boot for `record_ms` (5000–10000, default 8000) — the one allowed duration, and only for humans. Movement, animation, camera, and particles use `video`; a static UI change uses `screenshot`. Inherited regression checks are screenshots even if they say `video`.",
  ...
  "",
  "Verification hooks must require fields on the `EgonBridge` autoload (`EgonBridge.register_field`), read back as `window.__egon.state()` returning JSON. List every field the checks will read.",
  "",
