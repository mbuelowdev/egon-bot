...
export const PLANNER_INSTRUCTIONS = [
  ...
  "The Verification hooks section must specify the debug bridge as fields on the `EgonBridge` autoload, which already exists: `EgonBridge.register_field(\"name\", func(): return …)` per field, read back as `window.__egon.state()`. Name every field a check will read. Do not omit it, do not invent a different global, and do not ask for a hand-rolled JavaScriptBridge.",
