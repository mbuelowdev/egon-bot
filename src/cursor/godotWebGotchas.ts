...
export const GODOT_WEB_GOTCHAS_PROMPT = [
  ...
  "- Expose Verification hooks state with `get_node(\"/root/EgonBridge\").register_field(name, callable)` (or `$\"/root/EgonBridge\"`). `--check-only` does not load autoloads, so the `EgonBridge` identifier fails parse. The autoload already guards `OS.has_feature(\"web\")`, so registering is safe under headless `--quit-after`, and `get_node(\"/root/EgonBridge\").snapshot()` reads the same values without a browser. Hand-rolled JavaScriptBridge calls are not guarded and crash the headless smoke run.",
