...
- **Re-uploading identical bytes** under a different name resolves to the existing asset rather than creating a duplicate. Dropping a new file on an existing asset **replaces its bytes** and re-measures, keeping the id, description, tags, and grid; the extension has to match.
...
Sprite sheets are containers, not assets. A sheet carries a grid spec the human types — cell width, cell height — from which columns, rows, and frame count derive programmatically, which is exactly what Godot's `AtlasTexture` / `SpriteFrames` want. Per-row labels ("row 0 = walk down") go in the description as prose. Irregular atlases, 9-slice edges, and autotile blobs stay approximate: grid spec plus prose, and the implementer works out the bitmask.
